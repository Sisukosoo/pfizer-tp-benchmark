...
- Mixed portfolio: pharma activity significantly mixed with cosmetics, perfumes, devices, parapharmaceuticals, or own-marketed specialties.
- Low data quality: insufficient financial or business-description evidence to validate comparability.

The baseline result is 10 accepted comparables, 45 rejected candidates, and 0 pending candidates. User edits in the Streamlit app support sensitivity scenarios without overwriting the immutable baseline logic. This is consistent with the general principle in the OECD Transfer Pricing Guidelines that perfect comparables are often scarce in narrow markets and that a pragmatic, case-by-case comparability analysis is required.
...
- Tested-party characterization: LRD-SM.
- Scope: EBIT-level distribution benchmark; financial transactions scoped out.
- Primary PLI: Operating Margin.
- Base period: FY22-FY24.
- Sensitivity coverage: FY22 restructuring and Paxlovid dynamics.

The current comparable pool is methodologically useful but not perfect. Pfizer Pharma GmbH is substantially larger than the median comparable, and independent multinational pharmaceutical distributors are scarce in Europe. Several accepted comparables are Italian regional distributors, which may have lower margins than manufacturer-side distributors. The app therefore presents the result as a transparent TNMM benchmark with explicit limitations rather than as a mechanically definitive arm's-length conclusion.

## Reporting Output

The Excel workpaper is a presentation layer over the same calculation engine used in the app. It includes the tested-party overview, accepted comparables, rejected candidates, PLI detail, arm's-length range, sensitivity scenarios, and methodology notes. The report should be read as a reproducible learning workpaper based on the available Orbis export, not as a statutory transfer pricing report.
